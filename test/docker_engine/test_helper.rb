...
class DockerEngine::Test::SingletonClient < DockerEngine::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: DockerEngine::Test::SingletonClient::TEST_API_BASE_URL)
  end
end
...
class DockerEngine::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @docker = DockerEngine::Test::SingletonClient.instance
  end
