...
class DockerEngineAPI::Test::SingletonClient < DockerEngineAPI::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: DockerEngineAPI::Test::SingletonClient::TEST_API_BASE_URL)
  end
end
...
class DockerEngineAPI::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @docker = DockerEngineAPI::Test::SingletonClient.instance
  end
