...
class DockerEngineRuby::Test::SingletonClient < DockerEngineRuby::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: DockerEngineRuby::Test::SingletonClient::TEST_API_BASE_URL)
  end
end
...
class DockerEngineRuby::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @docker = DockerEngineRuby::Test::SingletonClient.instance
  end
