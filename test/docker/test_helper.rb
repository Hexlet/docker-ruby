...
class Docker::Test::SingletonClient < Docker::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Docker::Test::SingletonClient::TEST_API_BASE_URL)
  end
end
...
class Docker::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @docker = Docker::Test::SingletonClient.instance
  end
